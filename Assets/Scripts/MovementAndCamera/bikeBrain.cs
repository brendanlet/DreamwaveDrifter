...
public class bikeBrain : MonoBehaviour, IDamageable
{
    ...
    void FixedUpdate()
    {
        if (!isDead)
        {
            ...
            if (grappled)
            {
                Vector3 temp = grabtar.transform.position - transform.position;
                Vector3 temp2 = temp / temp.magnitude;
                line.transform.LookAt(grabtar.transform);
                line.SetActive(true);
                rb.AddForce(temp2 * jumpForce);
            }
            else
            {
                grabtar = null;
    ...
    private void Grapple()
    {
        ...
        if (Physics.Raycast(cam.transform.position, cam.transform.forward, out hit))
        {
            ...
            {
                grabtar = hit.collider.gameObject;
                grappled = true;
                swordSwingSource.Play();
                line.transform.LookAt(grabtar.transform);
